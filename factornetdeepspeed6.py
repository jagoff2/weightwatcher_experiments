...
class ExtendedModularArithmeticLayer(nn.Module):
    ...
    def forward(self, x, number_representation):
        ...
        for group_idx in range(self.num_groups):
            ...
            # Add small epsilon to avoid division by zero and ensure stable gradients
            eps = 1e-8
            normalized_residues = (residues / (group_moduli + eps)).to(x.dtype)
            # Scale down to prevent gradient explosion
            normalized_residues = torch.tanh(normalized_residues) * 0.5
...
class Factor512Net(nn.Module):
    ...
    def __init__(self, config):
        ...
        nn.init.constant_(self.halt_proj.bias, 0.1) # σ(0.5)=0.62  → E[iters]≈1.6
    ...
    def freeze_for_kickstart(self):
        """
        During 8-bit kick-start we keep gradients only for:
            * nybble_embed   (Module)
            * position_embed (Parameter)
            * input_proj     (Module)
            * mod_layer      (Module)
            * halt_proj      (Module)
        Everything else is temporarily frozen.
        """
        # 1. Freeze every param in the model
        for p in self.parameters():
            p.requires_grad = False

        # 2. Re-enable the small set we still want to learn
        trainable_objs = [
            self.nybble_embed,
            self.position_embed,   # NOTE: raw Parameter
            self.input_proj,
            self.mod_layer,
            self.halt_proj,
        ]
        for obj in trainable_objs:
    ...
    def unfreeze_all(self):
        "Re-enable gradients on every parameter (called after kick-start)."
        for p in self.parameters():
            p.requires_grad = True
        ...
        for module in self.modules():
            if isinstance(module, nn.Linear):
                ...
                if module == self.halt_proj:
                    ...
                    nn.init.constant_(module.bias, 0.5)  # Match the bias initialization above
                else:
...
# In Factor512Net.forward(), replace the entire ACT loop with:
        ...
        # ────────────────────────────────────────────────
        # ────────────────────────────────────────────────
        #  Adaptive-ACT compute budget
        #    • scale ≈ seq_len · log₂(bits)
        #    • clamp by config.act_max_iters
        # ────────────────────────────────────────────────
        bits = max(8, current_bit_size or 8)
        budget_scale = max(1, math.ceil(math.log2(bits)))            # 3..9 for 8-512 bits
        T_max = seq_len * budget_scale * self.config.act_base_multiplier // 8
        T_max = max(1, min(T_max, self.config.act_max_iters))
        ...
        for t in range(T_max):
            ...
            #----------------------------------------------------------
            # HOT-FIX: DeepSpeed ZeRO-3 + `torch.utils.checkpoint` on
            # Windows creates duplicated Parameter objects inside the
            #   params_in_ipg_bucket
            # list, tripping:
            #   assert len(set(p.ds_id …)) == len(bucket)
            #
            # Disable checkpointing by default.  You can re-enable it
            # by setting an env-var before launch:
            #     DS_ENABLE_CKP=1 python …/factornetdeepspeed5.py
            #----------------------------------------------------------
            use_ckpt = (
                os.getenv("DS_ENABLE_CKP", "0") == "1"
                and self.training
                and T_max > 16
            )

            h_new = (
                checkpoint(self.universal_block, h)
                if use_ckpt
                else self.universal_block(h)
            )
...
class Factor512Config:
    ...
    num_heads: int = 64
    act_multiplier: int = 64          # ← new (default doubles T_max)
    # Adaptive-ACT hyper-params (see Factor512Net.forward)
    act_base_multiplier: int = 8      # seq_len × 8 for ≤32-bit inputs
    act_max_iters: int = 512          # hard ceiling for any bit-width
    act_penalty_weight: float = 0.01  # loss coeff. for going over budget
    ...
    lr: float = 2e-2
    kickstart_lr: float = 3e-3        # gentler LR while only 5 small heads train
    weight_decay: float = 0.01
    max_steps: int = 200000
    
    # Learning rate schedule
    warmup_steps: int = 2000
    plateau_steps: int = 2000
...
# Scheduler removed - will use PyTorch LambdaLR instead
...
class ExtendedFactorizationLoss(nn.Module):
    ...
    def forward(self, pred_factor1, pred_factor2, true_factor1, true_factor2, 
        ...
        # ── PATCH S1-FIX: mask inactive nybbles (correct for small n) ─
        # ‣ For an N-bit semiprime the larger factor is ≤ ⌈N/2⌉ bits.
        # ‣ Each nybble stores 4 bits  →  nybble_count = ⌈⌈N/2⌉ / 4⌉.
        # ‣ Enforce **≥ 2 nybbles whenever N ≥ 8** so that every 8-bit
        #   semiprime trains on at least two nybbles (e.g. 2×3 = 6 still
        #   gets 2 nybbles because other 8-bit numbers need them).
        bit_lens    = torch.tensor([int(n).bit_length()
                                    for n in original_numbers],
                                   device=device)
        factor_bits = (bit_lens + 1) // 2           # ceil(N/2)
        active_len  = (factor_bits + 3) // 4        # ceil(/4) → nybbles

        # ensure minimum nybbles
        min_len     = torch.where(bit_lens >= 8,
                                  torch.tensor(2, device=device),
                                  torch.tensor(1, device=device))
        active_len  = torch.maximum(active_len, min_len)
        ...
        # Use a more reasonable maximum to avoid numerical issues
        SAFE_MAX = 1e100  # Still very large but avoids overflow in log computation
        def _safe_float(x: int) -> float:
            try:
                val = float(x)
                return min(val, SAFE_MAX)
        ...
        log_eps = 1e-10
        log_pred1 = torch.log(pred_nums1.clamp_min(1) + log_eps)
        log_pred2 = torch.log(pred_nums2.clamp_min(1) + log_eps)
        log_true  = torch.log(true_nums.clamp_min(1) + log_eps)

        # Avoid division by very small log_true values
        product_errors = torch.abs((log_pred1 + log_pred2) - log_true) / torch.clamp(log_true.abs(), min=0.1)
        ...
        if factor_loss.item() < 0.20 and self.current_product_weight == 0.0:
        ...
        # ── Disable product-error during 8-bit kick-start ───────────────
        kickstart_active = (
            self.config.kickstart_steps > 0 and
            max(original_numbers).bit_length() <= self.config.kickstart_bits
        )
        effective_prod_w = self.current_product_weight


        # Combine losses
        total_loss = (
            self.config.factor_loss_weight * factor_loss +
            effective_prod_w * product_error +
...
class RSA512DataGenerator:
    ...
    def generate_batch(self, batch_size, bit_size):
        ...
        # ───────────────  PATCH S-8bit  |  Exhaustive 8-bit semiprime sampler  ───────────────
        if bit_size == 8:
            if not hasattr(self, "_all_8bit_semiprimes"):
                full_list = []
                # 1️⃣ include prime 2  ➟ start at 2, not 3
                for p in range(2, 256):                       # every integer   (was range(3, 256, 2))
                    if not gmpy2.is_prime(p):
                        continue
                    # 2️⃣ step = 1  ➟ allow even q whenever p = 2
                    for q in range(p, 256 // p + 1):          # every integer   (was step 2)
                        if gmpy2.is_prime(q):
                            full_list.append((p * q, p, q))   # p ≤ q ⇒ no duplicates
        ...
        # ────────────────────────────────────────────────────────────────
        # Generate batch for non-8-bit cases
        numbers = []
        factors1 = []
        factors2 = []
        mod_zero_labels = []
        ...
        for _ in range(batch_size):
            n, p, q = self.generate_semiprime(bit_size)
            numbers.append(n)
            factors1.append(p)
            factors2.append(q)
            ...
            mod_zero, legendre = compute_modular_labels(n, FIRST_4096_PRIMES)
            mod_zero_labels.append(mod_zero)
            legendre_labels.append(legendre)
...
# Complete replacement for the train_step method and related initialization fixes
...
class Factor512TrainerDeepSpeed:
    ...
    def __init__(self, model: Factor512Net, config: Factor512Config):
        ...
        # CRITICAL: Do this FIRST before any other initialization
        ...
        # ── Kick-start flag must exist *before* we reference it ─────────
        self.in_kickstart = config.kickstart_steps > 0

        # If we are in the 8-bit kick-start, freeze most weights now,
        # *before* DeepSpeed grabs the parameter list.
        if self.in_kickstart:
            model.freeze_for_kickstart()
        ...
        # PATCH D3: Fix parameter list for optimizer to avoid duplicate registration
        # ------------------------------------------------------------------
        #  DEDUPLICATE PARAMETERS
        #  DeepSpeed ZeRO-3 asserts that every param in an IPG bucket is
        #  unique.  If a Parameter object shows up twice in the optimiser’s
        #  param list (e.g. tied weights or accidental duplicates), stage-3
        #  hits `len(set(p.ds_id …)) != len(bucket)` and aborts.  We guard
        #  against that by filtering on `id()` BEFORE handing anything to
        #  the optimiser / deepspeed.initialize.
        # ------------------------------------------------------------------
        ...
        if self.in_kickstart:
            # Only the small learnable subset
            param_list = _unique(p for p in model.parameters() if p.requires_grad)
            optimizer = DeepSpeedCPUAdam(
                param_list,
                lr=config.kickstart_lr,
                betas=(0.9, 0.98),
                eps=1e-9,
                weight_decay=config.weight_decay,
            )
        else:
            # Full-model training (no kick-start) – still dedup
            param_list = _unique(model.parameters())
            optimizer = DeepSpeedCPUAdam(
                param_list,
                lr=config.lr,
                betas=(0.9, 0.98),
        ...
        # ── 2. Build PyTorch scheduler with custom lambda ───────────
        # CRITICAL: The scheduler should step per OPTIMIZER step, not per backward pass
        lambda_fn = self._create_scheduler_lambda_fixed()
        ...
        # ── 3. Pass both objects to DeepSpeed ─────────────────────────────────
        # PATCH D4: Don't pass model_parameters when not in kickstart to avoid conflicts
        if self.in_kickstart:
            self.model_engine, self.optimizer, _, self.lr_scheduler = deepspeed.initialize(
                ...
                config=ds_config,
                model_parameters=param_list  # Use the same list we gave to optimizer
            )
        else:
            # Pass the SAME deduplicated list to DeepSpeed so ids stay aligned
            self.model_engine, self.optimizer, _, self.lr_scheduler = deepspeed.initialize(
                model=model,
                optimizer=optimizer,
                lr_scheduler=scheduler,
        ...
        # CRITICAL FIX: Track steps correctly with gradient accumulation
        self.micro_step = 0  # Counts every backward pass
        self.global_step = 0  # Counts every optimizer step (after accumulation)
        
        # Tracking variables with kickstart support
        self.step = 0  # This will track optimizer steps, not micro steps
    ...
    def _create_scheduler_lambda_fixed(self):
        """Create scheduler lambda function for LambdaLR that properly handles gradient accumulation"""
        def lr_lambda(optimizer_step):
            # CRITICAL: optimizer_step is the number of optimizer.step() calls,
            # NOT the number of backward passes
            
            # Adjust for kickstart phase
            if self.config.kickstart_steps > 0:
                # Kickstart steps are in terms of optimizer steps
                kickstart_optimizer_steps = self.config.kickstart_steps // self.config.gradient_accumulation_steps
                
                if optimizer_step < kickstart_optimizer_steps:
                    # During kickstart, use normal warmup
                    effective_step = optimizer_step
                else:
                    # After kickstart, restart the schedule
                    effective_step = optimizer_step - kickstart_optimizer_steps
            else:
                effective_step = optimizer_step
            
            # Adjust warmup/plateau/max steps for gradient accumulation
            warmup_optimizer_steps = self.config.warmup_steps // self.config.gradient_accumulation_steps
            plateau_optimizer_steps = self.config.plateau_steps // self.config.gradient_accumulation_steps
            max_optimizer_steps = self.config.max_steps // self.config.gradient_accumulation_steps
            
            # Warmup phase
            if effective_step < warmup_optimizer_steps:
                # Smooth quadratic ramp
                progress = effective_step / max(1, warmup_optimizer_steps)
                return progress * progress
            # Plateau phase
            elif effective_step < warmup_optimizer_steps + plateau_optimizer_steps:
                return 1.0
            # Cosine decay phase
            else:
                decay_steps = effective_step - warmup_optimizer_steps - plateau_optimizer_steps
                total_decay_steps = max(1, max_optimizer_steps - warmup_optimizer_steps - plateau_optimizer_steps)
                if self.config.kickstart_steps > 0:
                    total_decay_steps -= (self.config.kickstart_steps // self.config.gradient_accumulation_steps)
                return 0.5 * (1.0 + math.cos(math.pi * min(decay_steps / total_decay_steps, 1.0)))
        
        return lr_lambda
        
    def train_step(self):
        """Single training step with proper gradient accumulation handling"""
        ...
        # ===== ACT Monitoring Block =====
        # Calculate expected T_max based on current bit size
        expected_seq_len = min((self.current_bit_size + 3) // 4,
                            self.model_engine.module.max_nybbles)
        expected_T_max   = expected_seq_len * self.config.act_multiplier
        
        # Monitor ACT behavior on optimizer steps (not micro steps)
        if self.global_step % 100 == 0 and hasattr(self.model_engine.module, '_last_n_iters'):
            avg_iters = self.model_engine.module._last_n_iters
            avg_halt_prob = getattr(self.model_engine.module, '_last_halt_prob', 0.0)
            
            # Log ACT statistics
            logger.debug(f"ACT stats @ optimizer step {self.global_step}: avg_iters={avg_iters:.2f}/{expected_T_max}, "
                        f"first_halt_prob={avg_halt_prob:.3f}, bit_size={self.current_bit_size}")
            
            # Warn if ACT is not halting efficiently
            if avg_iters > expected_T_max * 0.9:
            ...
            self.act_efficiency_history.append({
                ...
                'efficiency': 1.0 - (avg_iters / expected_T_max),
        ...
        # ── ACT-overrun penalty ─────────────────────────────────────
        if hasattr(self.model_engine.module, "_last_n_iters"):
            iters = self.model_engine.module._last_n_iters
            seq_len = min((self.current_bit_size + 3) // 4,
                          self.model_engine.module.max_nybbles)
            expected = min(seq_len * self.config.act_base_multiplier,
                           self.config.act_max_iters)
            # Penalise only the excess iterations
            excess = max(0.0, iters - expected)
            penalty = self.config.act_penalty_weight * excess
            loss = loss + penalty
            loss_components["act_penalty"] = penalty
        
        # CRITICAL FIX: Scale loss by gradient accumulation steps
        # This ensures gradients are averaged correctly across accumulation steps
        ...
        # Track the unscaled loss for logging
        ...
        if self.model_engine.is_gradient_accumulation_boundary():
            ...
            # Increment global step (optimizer steps)
            self.global_step += 1
            self.step = self.global_step  # Keep self.step in sync
            
            # Log actual learning rate periodically
            if self.global_step % 50 == 0 and self.local_rank == 0:
                current_lr = self.lr_scheduler.get_last_lr()[0]
                logger.debug(f"LR after optimizer step {self.global_step}: {current_lr:.3e}")
        
        # Track timing
        step_time = time.time() - start_time
        self.step_times.append(step_time)
        
        return loss_value, loss_components, step_time
    
    def get_eval_interval(self):
        """Get evaluation interval based on current bit size (in optimizer steps)"""
        base_interval = self.config.eval_interval_small if self.current_bit_size <= 192 else self.config.eval_interval_large
        # Convert to optimizer steps
        return base_interval // self.config.gradient_accumulation_steps
    
    def run(self):
        """Main training loop with proper gradient accumulation handling"""
        ...
        if self.config.resume_from:
            ...
            logger.info(f"   Starting from optimizer step {self.step}")
        ...
        try:
            # CRITICAL: Loop over micro steps, not optimizer steps
            max_micro_steps = self.config.max_steps
            
            for micro_step in range(self.micro_step, max_micro_steps):
                self.micro_step = micro_step
                
                # Training step
                loss, loss_components, step_time = self.train_step()
                
                # Only track losses on optimizer steps to avoid memory bloat
                if self.model_engine.is_gradient_accumulation_boundary():
                ...
                # Evaluation - based on optimizer steps
                eval_interval = self.get_eval_interval()
                if self.global_step > 0 and self.global_step % eval_interval == 0 and self.local_rank == 0:
                    ...
                    # Detailed components every 1000 optimizer steps
                ...
                # Alpha monitoring - based on optimizer steps
                if self.global_step > 0 and self.global_step % self.config.alpha_interval == 0 and self.local_rank == 0:
                    ...
                    logger.info(f"📊 Alpha at optimizer step {self.global_step}: {mean_alpha:.3f}")
                    ...
                    if 2.1 <= mean_alpha <= 2.25 and self.current_bit_size >= 128:
                        self.save_checkpoint(f"alpha_{mean_alpha:.2f}_bits_{self.current_bit_size}_step_{self.global_step}")
                
                # Regular checkpointing - based on optimizer steps
                checkpoint_interval = self.config.checkpoint_interval // self.config.gradient_accumulation_steps
                if self.global_step > 0 and self.global_step % checkpoint_interval == 0 and self.local_rank == 0:
                    self.save_checkpoint(f"checkpoint_step_{self.global_step}_bits_{self.current_bit_size}")
                    
                    # Log average step time
                    if self.step_times:
                        avg_step_time = np.mean(self.step_times[-100:])
                        logger.info(f"  Average micro-step time: {avg_step_time:.3f}s")
                        
                        # Estimate time to completion
                        remaining_micro_steps = max_micro_steps - self.micro_step
                        eta_hours = (remaining_micro_steps * avg_step_time) / 3600
                ...
                if self.grokking_detected and not hasattr(self, 'grokking_saved') and self.local_rank == 0:
                    self.save_checkpoint(f"grokking_detected_step_{self.global_step}_bits_{self.current_bit_size}")
                ...
                if self.current_bit_size == 512 and self.success_rates and self.success_rates[-1] >= 0.9:
                    ...
                    if self.local_rank == 0:
                        self.save_checkpoint(f"512bit_success_step_{self.global_step}")
                    break
    ...
    def check_curriculum_advancement(self):
        """Check if ready to advance to next bit size (adjusted for optimizer steps)"""
        # Handle kick-start to main curriculum transition
        if self.in_kickstart:
            # Kickstart steps are in micro steps, convert to optimizer steps
            kickstart_optimizer_steps = self.config.kickstart_steps // self.config.gradient_accumulation_steps
            
            # Check if we've completed kickstart
            if self.global_step >= kickstart_optimizer_steps:
                # Also check for grokking
                if self.advancement_history and self.alphas:
                    ...
                    if grokking_ready:
                        ...
                        # Un-freeze & refresh optimizer
                        self.model_engine.module.unfreeze_all()
                        self.model_engine.refresh_trainable_params()
                        self.bit_level = 0
    ...
    def advance_curriculum(self):
        ...
        if self.bit_level >= 0 and self.bit_level < len(self.config.bit_curriculum) - 1:
            self.bit_level += 1
    ...
    def load_checkpoint(self, checkpoint_path: str):
        ...
        if client_state:
            ...
            self.global_step = client_state.get('global_step', self.step)
            self.micro_step = client_state.get('micro_step', self.step * self.config.gradient_accumulation_steps)
            ...
            if 'act_efficiency_history' in client_state:
                self.act_efficiency_history = client_state['act_efficiency_history']
            
        logger.info(f"✅ Checkpoint loaded successfully!")
        logger.info(f"  Resuming from step: {self.step}")
        logger.info(f"  Global step: {self.global_step}")
        logger.info(f"  Micro step: {self.micro_step}")
    ...
    def save_results(self):
        ...
        results = {
            ...
            'final_micro_step': self.micro_step,
    ...
    def _get_bit_size_progression(self):
        ...
        for i, rate in enumerate(self.success_rates):
            # Convert evaluation index to optimizer steps
            step = i * self.get_eval_interval()
            
            # Check if we transitioned from kickstart
            kickstart_optimizer_steps = self.config.kickstart_steps // self.config.gradient_accumulation_steps
            if self.config.kickstart_steps > 0 and step >= kickstart_optimizer_steps and len(progression) == 1:
                progression.append({
                    'step': step,
    ...
    def _get_performance_by_bits(self):
        ...
        if self.config.kickstart_steps > 0:
            ...
            for i, rate in enumerate(self.success_rates):
                step = i * self.get_eval_interval()
                if step < kickstart_optimizer_steps:
        ...
        for i, rate in enumerate(self.success_rates):
            ...
            kickstart_optimizer_steps = self.config.kickstart_steps // self.config.gradient_accumulation_steps
            if self.config.kickstart_steps > 0 and step < kickstart_optimizer_steps:
                continue
    ...
    def _save_csv_data(self):
        ...
        # Training data - note this is per optimizer step, not micro step
        df_train = pd.DataFrame({
            'optimizer_step': range(len(self.train_losses)),
        ...
        # Alpha data
        alpha_interval_optimizer_steps = self.config.alpha_interval // self.config.gradient_accumulation_steps
        alpha_steps = [i * alpha_interval_optimizer_steps for i in range(1, len(self.alphas) + 1)]
        df_alpha = pd.DataFrame({
            'optimizer_step': alpha_steps,
    ...
    def plot_results(self):
        ...
        ax.plot(train_steps, self.train_losses, label='Train Loss', alpha=0.7, linewidth=1)
        eval_steps = [i * self.get_eval_interval() for i in range(len(self.val_losses))]
        ...
        if self.config.kickstart_steps > 0:
            kickstart_optimizer_steps = self.config.kickstart_steps // self.config.gradient_accumulation_steps
            ax.axvline(x=kickstart_optimizer_steps, color='green', linestyle='--', 
...
Effective Batch Size: {self.config.batch_size * self.config.gradient_accumulation_steps}

Expected Memory Savings:
- Optimizer states: ~8x reduction
- Gradients: ~{self.world_size}x reduction
- Parameters: CPU offload enabled
        ...
        if self.loss_components:
            components_to_plot = ['factor_loss', 'product_error', 'residue_loss', 'legendre_loss', 'act_penalty']
...
Model params: {sum(p.numel() for p in self.model_engine.parameters())/1e6:.1f}M
        ...
        summary_lines = [
            ...
            f"Final optimizer step: {self.global_step:,}",
            f"Final micro step: {self.micro_step:,}",
            ...
            f"DeepSpeed: ZeRO-{self.config.zero_stage}, World Size: {self.world_size}",
            f"Gradient Accumulation: {self.config.gradient_accumulation_steps}x",
        ]
        
        if self.config.kickstart_steps > 0:
            summary_lines.append(f"Kickstart: {self.config.kickstart_steps} micro steps on {self.config.kickstart_bits}-bit")
